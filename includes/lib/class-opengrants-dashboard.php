...
function ogIframeEmbed($atts, $content = NULL)
{
	...
	if (is_user_logged_in()) {
		...
		if ($key) {
			...
			echo '<iframe src="http://localhost:3000/dashboard?logo=' . $logoUrl . '&primary=' . $primaryColorWithoutHex[1] . '&secondary=' . $secondaryColorWithoutHex[1] . '&user=' . urlencode(json_encode($authResponse)) . '" height="1000" width="1000" frameborder="0"></iframe>';
		} else {
